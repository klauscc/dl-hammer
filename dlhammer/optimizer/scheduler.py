...
def _get_warmup_factor_at_iter(warmup_method, this_iter, warmup_iters, warmup_factor):
...
class MultiStepLR(_LRScheduler):
    ...
    def get_lr(self):
        return [
            base_lr * self.gamma**bisect_left(self.milestones, self.last_epoch) for base_lr in self.base_lrs
        ]
...
class WarmupMultiStepLR(_LRScheduler):
    ...
    def get_lr(self):
        ...
        return [
            base_lr * warmup_factor * self.gamma**bisect_left(self.milestones, self.last_epoch)
...
class CosineLR(_LRScheduler):
    ...
    def __init__(self, optimizer, max_iters, last_epoch=-1):
        self.max_iters = max_iters
        super().__init__(optimizer, last_epoch)

    def get_lr(self):
        return [
            base_lr * 0.5 * (1.0 + math.cos(math.pi * self.last_epoch / self.max_iters))
            for base_lr in self.base_lrs
        ]
...
def build_scheduler(start_epoch, num_epochs, scheduler_cfg, optimizer, num_iters_per_epoch):
    ...
    if 'NAME' in scheduler_cfg:
        scheduler_cfg.SCHEDULER_NAME = scheduler_cfg['NAME']
    if scheduler_cfg.SCHEDULER_NAME == 'multistep':
    ...
    elif scheduler_cfg.SCHEDULER_NAME == 'warmup_multistep':
        # Convert milestones epochs to iters
        milestones = [(i - 1) * num_iters_per_epoch for i in scheduler_cfg.MILESTONES]
    ...
    elif scheduler_cfg.SCHEDULER_NAME == 'cosine':
        # Get max number of iters
        max_iters = num_epochs * num_iters_per_epoch

        scheduler = CosineLR(optimizer, max_iters=max_iters, last_epoch=last_epoch)
    elif scheduler_cfg.SCHEDULER_NAME == 'warmup_cosine':
        # Get max number of iters
        max_iters = num_epochs * num_iters_per_epoch
        ...
        raise RuntimeError('Unknown lr scheduler: {}'.format(scheduler_cfg.SCHEDULER_NAME))
    return scheduler
